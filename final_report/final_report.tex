...
trade-off, document the regimes where the LLM prior helps and hurts, and discuss why SVGD
becomes the natural inference engine in higher-dimensional extensions of this template.
\end{abstract}
...
information about the behavior we are trying to estimate. The natural question for
...
posterior dependence. Our pipeline uses SVGD to demonstrate the design pattern; we
honestly report below that for the two-parameter Beta posterior, MCMC is faster
per equivalent ESS, and discuss why SVGD becomes attractive when the model dimension grows.
...
Phillip's check-in feedback flagged that cross-game consistency of $\rho$ is the strongest
generalization claim available. We re-elicited 500 pseudo-observations from GPT-4o under the
...
\noindent
The result is striking and nuanced. \emph{Sample-size reduction is identical:}
...
ablation cleanly disentangles two effects. (i) Power-prior \emph{variance shrinkage} is
governed by the prior's effective weight $w$ and is therefore essentially independent of
...
\paragraph{Why does the LLM prior help most at small $n$?}
...
prediction $(50+20)/50 = 1.4$. The LLM prior is essentially a free $w$-observation pre-fix.
...
\paragraph{Why SVGD here, when MCMC wins on ESS/sec?}
The Pareto analysis (Figure~\ref{fig:pareto}) is honest: at $d = 2$ for a unimodal Beta
posterior, MH-MCMC mixes well, costs $O(1)$ per sample, and produces ESS/sec roughly
