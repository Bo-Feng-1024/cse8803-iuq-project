...
for cents. The UQ question is: can we treat the LLM as an informative Bayesian prior?
That's the project.}
\end{frame}
...
\end{column}
\end{columns}
\note{The check-in's strongest suggestion was to do this on the taking game. Result: rho
equals zero point five five, twice as high. Why? GPT-4o under the taking prompt has
counterpart-kept share of zero point five two, but humans in the taking game keep only
zero point three four. Humans take more aggressively than the LLM. The LLM generalises
the protocol but not the framing-induced behavioural shift.}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item $\delta \ge +1.75$: data dominates
\end{itemize}
\vspace{0.5em}
...
\end{column}
\end{columns}
\note{The list. The biggest one: I never actually tested SVGD in the regime where it
would dominate MCMC, because I only ever inferred two parameters. The extension is
a hierarchical model across both game variants. That gets you to five parameters
with correlations, and is exactly the d-regime where SVGD's particle update would
amortise.}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{6pt}
  ...
  \item SVGD vs MCMC at $d=2$: MCMC delivers 100$\times$ higher ESS/sec; SVGD's value awaits the higher-d hierarchical extension.
\end{itemize}
\vspace{0.6em}
\centering\small
...
misspecified upward shifts, the leakage ablation separates variance shrinkage from
prior-location quality, and SVGD waits for the higher-dimensional sequel. Thanks;
